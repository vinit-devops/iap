...
- **`@iap/cli`** (bin `iap`) — self-contained zero-dependency npm package built by
  `tools/packaging/build-cli.mjs` into `dist-pkg/cli`. `npm publish --dry-run` clean.
- **`@iap/mcp-server`** (bin `iap-mcp-server`) — self-contained zero-dependency npm
...
- **Claude Code plugin** (`.claude-plugin/` + `commands/iap-*.md`) — marketplace +
  ...
  proven; the committed form launches `npx -y @iap/mcp-server` (pending publish).
...
- **Public identities:** npm `@iap/cli` (bin `iap`) and `@iap/mcp-server`
  (bin `iap-mcp-server`); VS Code Marketplace publisher `iap`; OpenVSX namespace
  `iap`; public repo `https://github.com/vinit-devops/iap`.
- **Known naming risk:** unscoped `iap` on npm is taken by a third party (v1.1.1),
  and the availability of the `@iap` org cannot be verified without an npm login.
  The runbook carries a concrete fallback rename (`iap-cli` / `iap-mcp-server`,
  both verified 404/available on 2026-07-15).
...
| M20.4     | Claude Code plugin                         | prepared (⛔ publish)   | `.claude-plugin/plugin.json` + `marketplace.json` validate clean (incl. `--strict`); isolated-env install from local path proven (`docs/reports/evidence/m20.6/claude-code-plugin-install.md`); committed form needs `@iap/mcp-server` on npm (runbook 6) |
...
| Claude Code (plugin)     | PENDING-publish       | Marketplace add + install + list proven locally; bundled `npx -y @iap/mcp-server` cannot connect until the package exists on npm (identical binary passes locally)              |
...
- **Nothing is published.** No npm package, no Marketplace/OpenVSX listing, no
  ...
  not a live artifact.
- **The `@iap` npm scope is unverified.** It may be unavailable; the fallback
  rename path in the runbook is untested end-to-end (though both fallback names
  were 404/available on 2026-07-15).
